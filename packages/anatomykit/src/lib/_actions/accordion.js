import { addListener, getElementIndex, selectNext, selectPrevious } from './_helpers.js';
...
export function accordion(node, params) {
	...
	let selected = matchingValue > -1 ? matchingValue : 0;
	...
	function setup() {
		Array.from(node.children).forEach((element, index) => {
			...
			if (!contents) {
				throw new Error("each accordion item requires a content element with the role of 'region'");
			}
	...
	return {
		...
		update(params) {
			...
			selected = matchingValue > -1 ? matchingValue : selected;
			updateChild(selected);
		},
		destroy() {
